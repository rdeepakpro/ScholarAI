...
function LocalAIContent({ controller }: { controller: LocalAIController }) {
  ...
  useEffect(() => {
    window.speedyAI?.provider.get().then((value) => {
  ...
  if (state.status === 'downloading' || state.status === 'verifying') return <section className="ai-download-panel">
    ...
    <p>{state.status === 'verifying' ? 'Making sure every byte arrived safely before installation.' : 'You can keep using SpeedyAI while this finishes.'}</p>
  ...
  return <>
    {ready ? <section className="settings-section ai-ready">
      ...
      <div className="settings-icon"><Cpu /></div><p className="kicker">RECOMMENDED</p><h2>Run AI privately on this computer.</h2><p className="settings-lead">No account, API key, or separate software required. Download once, then use SpeedyAI offline.</p>
      ...
      {!window.speedyAI && <p className="desktop-note"><Monitor size={15} />Downloads become available in the installed desktop app.</p>}
    </section>}
...
export function SettingsView({ controller, theme, onTheme, data, onChange }: { controller: LocalAIController; theme: Theme; onTheme: (theme: Theme) => void; data: AppData; onChange: (fn: (data: AppData) => AppData) => void }) {
  ...
  return <div className="page settings-page"><div className="settings-header"><h1>Settings</h1></div><div className="settings-layout"><nav className="settings-nav">{tabs.map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}><Icon size={17} />{label}</button>)}</nav><div className="settings-content">
    {tab === 'models' && <><div className="settings-content-title"><h1>Models</h1><p>Choose how SpeedyAI generates lessons, notes, flashcards, and quizzes.</p></div><LocalAIContent controller={controller} /></>}
    ...
    {tab === 'data' && <><div className="settings-content-title"><h1>Data</h1><p>Your study data stays on this computer.</p></div><section className="settings-section data-actions"><div><strong>Local data location</strong><p>SpeedyAI application data directory</p></div><button className="button secondary" onClick={() => exportData(data)}>Export my data</button><button className="button tertiary" onClick={() => { if (window.confirm('Delete all local SpeedyAI data? This cannot be undone.') && window.confirm('Are you absolutely sure? Export first if you need a copy.')) { clearData(); location.reload() } }}><Trash2 size={15} />Delete all local data</button></section></>}
    {tab === 'about' && <><div className="settings-content-title"><h1>About</h1></div><section className="settings-section about-panel"><h2>SpeedyAI</h2><p>Version 0.1.0</p><p>Open-source desktop study application.</p><a href="https://github.com" target="_blank" rel="noreferrer">GitHub</a><a href="https://opensource.org/license/mit" target="_blank" rel="noreferrer">MIT License</a></section></>}
  </div></div></div>
}
